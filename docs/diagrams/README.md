...
20개 요소와 28개 연결선이 하나의 그래프로 이어진다. 아래쪽에는 외부 개인 기기 → Tailscale VPN → 홈 LAN 경로와 외부 Tailscale 관리 서비스를 둔다. Argo CD는 VPN 배포 선언을 동기화하고, Tailscale 관리 서비스는 계정·기기 등록·접근 정책을 담당한다. 실제 mxGraph 렌더링으로 상자·라벨 배치를 확인했다. PNG 미리보기는 3540×2350이다.
